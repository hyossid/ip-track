...
var (
	...
	user= flag.String("user","sidney","Name of current user executing script")
)
...
func init() {
	...
	log.Printf("Initiating DB, using mmdb")
	db, err := maxminddb.Open("data/country.mmdb")

	if err != nil {
		log.Fatalf("Failed to initiate", err)
		panic(err)
	}
	geoIp = db
}
...
func main() {
	...
	h := requestHandler
	if *compress {
		h = fasthttp.CompressHandler(h) //  transparently compresses response body generated by h if the request contains 'gzip' or 'deflate' 'Accept-Encoding' header
	}
